...
function MojiProjekti() {
  ...
  const handleNavigateToProject = (projekt) => {
    navigate(`/moji-projekti/${projekt._id}`); 
  };
        ...
        <ul>
          {projektLoading ? (
            ...
            projekti.map((projekt, index) => (
              <li
                key={index}
                onClick={() => handleNavigateToProject(projekt)}
              ...
              >
                <div className="flex items-center space-x-3">
                  <span
                    className={`w-7 h-7 flex-shrink-0 rounded-full ${getRandomColor(
                      index
